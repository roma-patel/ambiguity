...
def prepare_data(fold, data, word_vecs):
    ...
    train_words = X_train; test_words = X_test
    print test_words
...
def train(X_train, y_train, X_test, y_test, train_words, test_words):

    log_reg = LogisticRegression()
    log_reg.fit(X_train, y_train)

    #X_train = [[np.random.rand(1)[0] for i in range(1, 100)] for item in words]; 
    ...
    if os.path.isdir(dirpath) is False: os.mkdir(fpath)
...
if __name__ == '__main__':
    ...
    for fold in range(0, 10):
        ...
        train(X_train, y_train, X_test, y_test, fold, train_words, test_words)
