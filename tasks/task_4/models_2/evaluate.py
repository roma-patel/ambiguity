...
path = '/nlp/data/romap/law/task_4/models_2/shell/'
            ...
            for wvec in wvecs:
                ...
                if len(lines) > 2:
                    ...
                    for line in lines[1:11]:
                        items = line.strip().split('\t')
                        prec.append(float(items[0])); recl.append(float(items[1]))
                        acc.append(float(items[2])); fm.append(float(items[3]))
                        
                    #items = lines[1].strip().split('\t'); items = [round(float(item), 2) for item in items]
                    items = [0, round(np.mean(prec), 2), round(np.mean(recl), 2), round(np.mean(acc), 2), round(np.mean(fm), 2)]
