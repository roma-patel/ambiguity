...
def createTitleFeat(set_names, cases):
    ...
    for i in range(len(cases)):
        ...
        titles = [0.0 for i in range(0, 54)]; count = 0
...
def prepare_features(X_train, y_train, X_test, y_test):
    ...
    #title features
    try:
        fileObject = open(path + "svm/features/feat/train_title_feat.p", "r")
        train_title_feat = pickle.load(fileObject)
        fileObject.close()

    except IOError as e:
        train_title_feat = createTitleFeat(train_title_paths, X_train)
        fileObject = open(path + "svm/features/feat/train_title_feat.p", "wb")
        pickle.dump(train_title_feat, fileObject)
        fileObject.close()
    try:
        fileObject = open(path + "svm/features/feat/test_title_feat.p", "r")
        test_title_feat = pickle.load(fileObject)
        fileObject.close()

    except IOError as e:
        test_title_feat = createTitleFeat(test_title_paths, X_test)
        fileObject = open(path + "svm/features/feat/test_title_feat.p", "wb")
...
#return lists of fileids
def get_fileids():
    X_train, y_train, X_test, y_test, X_dev, y_dev = [], [], [], [], [], []
    
    train = path + 'fileids/' + set_name + '/'
    f = open(train + 'positive.txt', 'r')
    for line in f:
        X_train.append(line.split('\t')[0]); y_train.append(1)

    f = open(train + 'negative.txt', 'r')
    for line in f:
        X_train.append(line.split('\t')[0]); y_train.append(0)
        
    #first 100 +ve and last 800 -ve
    X_dev = X_train[:100]; X_dev.extend(X_train[-100:])
    y_dev = y_train[:100]; y_dev.extend(y_train[-100:])

    X_train = X_train[100:]; y_train = y_train[100:]
    X_train = X_train[:-100]; y_train = y_train[0:-100]

    test = path + 'fileids/test/'
    f = open(test + 'positive.txt', 'r')
    for line in f:
        X_test.append(line.split('\t')[0]); y_test.append(1)

    f = open(test + 'negative.txt', 'r')
    for line in f:
        X_test.append(line.split('\t')[0]); y_test.append(0)
        

    print 'train'; print X_train; print len(X_train); print '\n\n\n\n'
    print 'test'; print X_test; print len(X_test); print '\n\n\n\n'
    print 'dev'; print X_dev; print len(X_dev); print '\n\n\n\n'
